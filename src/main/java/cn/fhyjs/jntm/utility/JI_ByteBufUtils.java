...
/**
 * 用于与{@link ByteBuf}交互的实用程序。
 * @author cpw fhyjs
 *
 */
public class JI_ByteBufUtils {
    /**
     * 使用7位VARINT编码写入提供的INT的字节数。
     * @param toCount 要分析的数字。
     * @return 写入字节数(最多5个)
     */
    ...
    /**
     * 从字节缓冲区读取UTF8字符串。
     * 编码为<varint long>[<UTF8 char bytes>]。
     * @param from 要从中读取的缓冲区
     * @return 字符串
     */
    public static String readUTF8String(ByteBuf from)
    {
        int len = readVarInt(from,5);
    ...
    public static void writeUTF8String(ByteBuf to, String string)
    {
        ...
        Validate.isTrue(varIntByteCount(utf8Bytes.length) < 6, "The string is too long for this encoding.");
        writeVarInt(to, utf8Bytes.length, 5);
