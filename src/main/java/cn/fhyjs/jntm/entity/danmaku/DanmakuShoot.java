...
public final class DanmakuShoot {
     ...
     * @param color      弹幕颜色
     */
    public static void posedShot(World worldIn, EntityLivingBase thrower, double x,double y,double z,float h, float damage, float gravity, Float velocity, Float inaccuracy, DanmakuType type, int color) {
        if (!worldIn.isRemote) {
            Ta_Danmaku danmaku = new Ta_Danmaku(worldIn, thrower, damage, gravity, type, color);
            float offset = 0.3f / h;
            danmaku.shoot(x - thrower.posX, y - thrower.posY - offset, z - thrower.posZ, velocity, inaccuracy);
     ...
     * @param color      弹幕颜色
     */
    public static void aimedShot(World worldIn, EntityLivingBase thrower, EntityLivingBase target, float damage, float gravity, Float velocity, Float inaccuracy, DanmakuType type, int color) {
        if (!worldIn.isRemote) {
            Ta_Danmaku danmaku = new Ta_Danmaku(worldIn, thrower, damage, gravity, type, color);
     ...
     */
    public static void fanShapedShot(World worldIn, EntityLivingBase thrower, EntityLivingBase target, float damage, float gravity, Float velocity, Float inaccuracy, DanmakuType type, int color, double yawTotal, int fanNum) {
