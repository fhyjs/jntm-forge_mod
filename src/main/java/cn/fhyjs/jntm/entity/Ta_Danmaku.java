//改自https://github.com/TartaricAcid/TouhouLittleMaid/blob/1.12.2/src/main/java/com/github/tartaricacid/touhoulittlemaid/entity/projectile/EntityDanmaku.java
...
import static cn.fhyjs.jntm.entity.Ta_Danmaku.NBT.*;

public class Ta_Danmaku extends EntityThrowable {
    private static final int MAX_TICKS_EXISTED = 200;
    private static final DataParameter<Integer> TYPE = EntityDataManager.createKey(Ta_Danmaku.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> COLOR = EntityDataManager.createKey(Ta_Danmaku.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE = EntityDataManager.createKey(Ta_Danmaku.class, DataSerializers.FLOAT);
    private static final DataParameter<Float> GRAVITY = EntityDataManager.createKey(Ta_Danmaku.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> TICKS_EXISTED = EntityDataManager.createKey(Ta_Danmaku.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> DAMAGES_TERRAIN = EntityDataManager.createKey(Ta_Danmaku.class, DataSerializers.BOOLEAN);

    public Ta_Danmaku(World worldIn) {
        super(worldIn);
    }
     ...
     * @param color     弹幕颜色
     */
    public Ta_Danmaku(World worldIn, EntityLivingBase throwerIn, DanmakuType type, DanmakuColor color) {
     ...
     * @param color     弹幕颜色
     */
    public Ta_Danmaku(World worldIn, EntityLivingBase throwerIn, float damage, float gravity, DanmakuType type, int color) {
        super(worldIn, throwerIn);
        this.getDataManager().set(TYPE, type.ordinal());
    ...
    protected void onImpact(@Nonnull RayTraceResult result) {
        if (result.typeOfHit == RayTraceResult.Type.ENTITY) {
            boolean throwerMaidHasSasimono = getThrower() instanceof EntityLiving;
            boolean hitMaidHasSasimono = result.entityHit instanceof EntityLiving;
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound compound) {
        ...
        compound.setInteger(DANMAKU_COLOR.getName(), getColor());
